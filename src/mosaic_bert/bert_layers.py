...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.register_buffer("token_type_ids", torch.zeros(config.max_position_embeddings, dtype=torch.long), persistent=False)
...
class BertUnpadSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if (not IMPL_USE_FLASH2 and self.p_dropout) or flash_attn_qkvpacked_func is None:
            ...
            if IMPL_USE_FLASH2:
                ...
                if convert_dtype:
                    ...
                    attention = flash_attn_qkvpacked_func(qkv, dropout_p=self.p_dropout, alibi_slopes=slopes)
        ...
        attention = bert_padding_module.unpad_input_only(attention, torch.squeeze(attn_mask) == 1)  # type: ignore
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        """
        assert (bias is None) == (slopes is None), f"{bias=}, {slopes=}"
        attention_output = self.attention(hidden_states, cu_seqlens, seqlen, subset_idx, indices, attn_mask, bias, slopes)
    ...
    def forward(
        ...
        **kwargs,
        ...
        encoder_outputs = self.encoder(
            embedding_output, attention_mask, output_all_encoded_layers=output_all_encoded_layers, subset_mask=subset_mask
        )
